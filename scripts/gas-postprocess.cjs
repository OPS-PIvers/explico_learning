...
class GASPostProcessor {
  constructor(inputFile, outputFile) {
    ...
    this.extractedGASCode = null; // Store the complete clean GAS code
  }

  /**
   * Extract function implementations from webpack bundle
   * The gas-webpack-plugin generates clean GAS code at the end of the bundle
   */
  extractFunctions(content) {
    // Find the clean GAS code section generated by gas-webpack-plugin
    const startMarker = '// Main entry point for Google Apps Script';
    const endMarker = '__webpack_require__.g.';
    
    const startIndex = content.indexOf(startMarker);
    if (startIndex === -1) {
      console.warn('⚠️  Could not find GAS code section start marker');
      this.generateAllStubs();
      return;
    }
    
    // Find where the webpack global assignments start (end of clean functions)
    const endIndex = content.indexOf(endMarker);
    if (endIndex === -1) {
      console.warn('⚠️  Could not find GAS code section end marker');
      this.generateAllStubs();
      return;
    }
    
    // Extract the clean GAS code section
    const gasSection = content.substring(startIndex, endIndex).trim();
    console.log(`✅ Found clean GAS code section (${gasSection.length} characters)`);
    
    // Instead of parsing individual functions, use the entire clean section
    // This avoids duplication issues and ensures all code is preserved
    this.extractedGASCode = gasSection;
    
    console.log(`✅ Extracted complete clean GAS code section`);
  }

  /**
   * Generate all functions as stubs (fallback)
   */
  generateAllStubs() {
    ...
    for (const funcName of targetFunctions) {
      ...
      console.log(`⚠️  Using stub for: ${funcName}`);
    }
  }

  /**
   * Extract a single function from clean GAS code
   */
  extractSingleFunction(code, functionName) {
    // Look for the specific function by name with word boundaries
    const functionPattern = new RegExp(`(\\/\\*\\*[\\s\\S]*?\\*\\/\\s*)?function\\s+${functionName}\\s*\\([^)]*\\)\\s*\\{`, 'g');
  ...
  /**
   * Clean up extracted function code - SIMPLIFIED for clean GAS input
   */
  cleanupFunction(functionCode, functionName) {
    let cleaned = functionCode
      // Remove any remaining webpack artifacts (shouldn't be any in clean section)
    ...
    // The code should already be properly formatted from gas-webpack-plugin
    return cleaned;
  }
...
`;
    ...
    // If we have extracted clean GAS code, use it directly
    if (this.extractedGASCode) {
      return header + this.extractedGASCode;
    }

    // Fallback to individual functions or stubs
    const targetFunctions = [
      'doGet', 'include', 'getProjects', 'createProject', 'deleteProject',
      'getProjectData', 'saveHotspots', 'saveSlides', 'createProjectSpreadsheet',
      'setupSpreadsheetStructure'
  ...
  async process() {
    try {
      ...
      if (this.extractedGASCode) {
        console.log(`📁 Output size: ${(output.length / 1024).toFixed(1)}KB (complete clean section)`);
      } else {
